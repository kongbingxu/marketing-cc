package com.br.marketing.sync.monitor;
...
/**
 * sync服务Job开关守护任务
 * 扫描syncJobOnlineSwitch开关，如果开启超过30分钟则发送告警
...
@Component
@Slf4j
public class SyncJobSwitchGuardJob extends AbstractSimpleElasticJob {
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        try {
            // 获取开关状态
            Boolean jobOnlineSwitch = marketingCommonConfig.getSyncJobOnlineSwitch();
            ...
            if (isSwitchOpen) {
                ...
                if (getSwitchOpenTime() == null) {
                    ...
                    log.info("检测到sync服务Job开关已开启，开始计时");
                    return;
                }
                ...
                if (openDuration >= ALARM_THRESHOLD_MS) {
                    ...
                    String alarmMessage = String.format(
                        "【重要告警】sync服务Job上线开关(syncJobOnlineSwitch)已开启超过%d分钟！" +
                    ...
                    long minutes = openDuration / (60 * 1000);
                    log.warn("sync服务Job开关已开启{}分钟，未达到告警阈值", minutes);
                }
            } else {
                // 开关已关闭
            ...
            log.error("sync服务Job开关守护任务执行异常", e);
        }
    }
}
