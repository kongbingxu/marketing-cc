...
public class AlarmAppender<E> extends RollingFileAppender<E>  {
    ...
    protected void subAppend(E eventObject) {
        ...
        if(eventObject instanceof LoggingEvent){
            ...
            try {
                SystemExceptionServiceImpl emailService= MarketingDataRelayServiceApplication.ac.getBean(SystemExceptionServiceImpl.class);
                emailService.sendAlarmPrintStack(loggerName+":</br>"+content.toString(), "MARKETING-DATA-RELAY-SERVICE",throwableProxy);
            } catch (Exception e) {
