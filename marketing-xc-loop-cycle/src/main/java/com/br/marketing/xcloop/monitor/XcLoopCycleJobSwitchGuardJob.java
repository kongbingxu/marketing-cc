package com.br.marketing.xcloop.monitor;
...
/**
 * xc-loop-cycle服务Job开关守护任务
 * 扫描xcLoopCycleJobOnlineSwitch开关，如果开启超过30分钟则发送告警
 * 注意：此Job在monitor包下，不受JobSwitchXcLoopCycleAspect切面影响
 * 
 * @author system
 */
@Component
@Slf4j
public class XcLoopCycleJobSwitchGuardJob extends AbstractSimpleElasticJob {
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        try {
            // 获取开关状态
            Boolean jobOnlineSwitch = marketingCommonConfig.getXcLoopCycleJobOnlineSwitch();
            ...
            if (isSwitchOpen) {
                ...
                if (openDuration >= ALARM_THRESHOLD_MS) {
                    ...
                    log.warn(AlertLog.buildWarnMessage(
                        ...
                        alarmMessage,
                        "xc-loop-cycle Job开关超时告警"
                    ));
                } else {
                    long minutes = openDuration / (60 * 1000);
                    log.warn("xc-loop-cycle Job开关已开启{}分钟，未达到告警阈值", minutes);
                }
            } else {
                // 开关已关闭
