...
@Component
@Slf4j
public class SmyCustomizedUploadDataCleanJob extends AbstractSimpleElasticJob {
    @Resource
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        ...
        String apiCode = smyCustomizeDataConfig == null ? "3710196" : smyCustomizeDataConfig.getString("uploadApiCode");
        ...
        log.warn("萨摩耶定制上传数据清洗开始");
        long start = System.currentTimeMillis();
        smyDataCleanService.cleanCustomizedUploadData(apiCode, date);
        long end = System.currentTimeMillis();
        log.warn("萨摩耶定制上传数据清洗结束，耗时：" + (end - start));
