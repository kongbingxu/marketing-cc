package com.br.marketing.xc.monitor;
...
/**
 * xc-general服务Job开关守护任务
 * 扫描xcGeneralJobOnlineSwitch开关，如果开启超过30分钟则发送告警
 * 注意：此Job不受JobSwitchXcGeneralAspect切面影响，即使开关开启也能正常执行
 * 
 * @author system
 */
@Component
@Slf4j
public class XcGeneralJobSwitchGuardJob extends AbstractSimpleElasticJob {
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        try {
            // 获取开关状态
            Boolean jobOnlineSwitch = marketingCommonConfig.getXcGeneralJobOnlineSwitch();
            ...
            if (isSwitchOpen) {
                ...
                if (getSwitchOpenTime() == null) {
                    ...
                    log.info("检测到xc-general Job开关已开启，开始计时");
                    return;
                }
                ...
                if (openDuration >= ALARM_THRESHOLD_MS) {
                    ...
                    String alarmMessage = String.format(
                            "【重要告警】xc-general服务Job上线开关(xcGeneralJobOnlineSwitch)已开启超过%d分钟！" +
                    ...
                    log.warn(AlertLog.buildWarnMessage(
                            ...
                            alarmMessage,
                            "xc-general Job开关超时告警"
                    ));
                } else {
                    long minutes = openDuration / (60 * 1000);
                    log.warn("xc-general Job开关已开启{}分钟，未达到告警阈值", minutes);
                }
            } else {
                // 开关已关闭
            ...
            log.error("xc-general Job开关守护任务执行异常", e);
        }
    }
}
