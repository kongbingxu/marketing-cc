...
@Service
@Slf4j
public class WuBaRuleCollectDataImpl extends CommonMethodHandlerService {

    @Override
    @SuppressWarnings("unchecked")
    public void ruleNecessaryData(List transmitFacts, ProcessHandlerContext context) {
        if (!transmitFacts.isEmpty() && transmitFacts.get(0) instanceof MarketingTransferSyncUser) {
            WuBaRuleNecessaryData data = new WuBaRuleNecessaryData();
    ...
    public RuleDataCollectionEnum label() {
        return RuleDataCollectionEnum.WUBA_TRANSFER_FILTER_COLLECTION;
    }


    @EqualsAndHashCode(callSuper = true)
    @Data
    public static class WuBaRuleNecessaryData extends RuleNecessaryData {
        /**
         * 状态
         */
