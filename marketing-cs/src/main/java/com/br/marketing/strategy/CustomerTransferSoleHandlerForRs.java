...
@Service
@Slf4j
public class CustomerTransferSoleHandlerForRs extends AbstractExternalInterfaceHandler<ConversionDataDTO> {
    ...
    public JSONObject call(List<ConversionDataDTO> transferList, ProcessHandlerContext context) {
        ...
        // 遍历数据数组
        for (ConversionDataDTO conversionDataDTO : transferList) {
            String apiCode = conversionDataDTO.getApiCode();
        ...
        long timeConsuming = (System.currentTimeMillis() - start)/1000;
        if(timeConsuming>5){
            log.warn("推送客服转化去重耗时s:{}",timeConsuming);
        }
        return null;
    }
    ...
    public InterfaceHandlerEnum handlerEnum() {
        return InterfaceHandlerEnum.CUSTOMER_AUTO_FILTRATION_SOLE_RS;
    }
}
