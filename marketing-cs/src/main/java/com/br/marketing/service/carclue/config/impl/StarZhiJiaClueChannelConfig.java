...
/**
 * @ClassName StarZhiJiaClueChannelConfig
 * @Author kongbx
 * @Date 2025/5/9 10:16
 */
@Service
@Slf4j
public class StarZhiJiaClueChannelConfig extends AbstractClueChannelConfig {
    ...
    public void verifyCarClueInit(StringBuilder stringBuilder, CarClueRelationalMapping carClueRelationalMapping,
        ...
        // 字典表中品牌未直接匹配到，需要匹配子品牌
        List<CarClueSeriesInformation> subBrandList = subBrandNameMap.get(carClueInitMapping.getBrandName());
        // 品牌映射中直接找到匹配项
        if (!CollectionUtils.isEmpty(subBrandList)) {
            carClueRelationalMapping.setBrandId(subBrandList.get(0).getBrandId());
            carClueRelationalMapping.setBrandName(subBrandList.get(0).getBrandName());
            return;
        }
    ...
    @Override
    public String label() {
        return ChannelRule.ConfigChannelRuleEnum.ZJ_CONFIG.getLabel();
    }
}
