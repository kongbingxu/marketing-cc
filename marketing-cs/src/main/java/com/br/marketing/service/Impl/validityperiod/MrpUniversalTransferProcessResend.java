...
@Slf4j
@Service
@ValidityPeriodResendType(resendType = ValidityPeriodResendEnum.MRP_UNIVERSAL_TRANSFER_PROCESS_RESEND)
public class MrpUniversalTransferProcessResend implements ValidityPeriodResendStrategy<MarketingTransferInfo> {
    ...
    public void resend(List<MarketingTransferInfo> data, ValidityPeriodResendRecord record) {
        ...
        data.stream().map(transferInfo -> buildMrpMqFact(transferInfo, record))
            .forEach((MrpMqFact mqFact) -> pool.submit(() -> {
                if(rocketMqSwitch.rocketMQSwitchFlag(null, MarketingTransferConstants.TAG_MARKETING_MRP_UNIVERSAL_TRANSFER_RECEIVE)){
                    rocketMqSwitch.syncSend(MarketingTransferConstants.TOPIC
                            , MarketingTransferConstants.TAG_MARKETING_MRP_UNIVERSAL_TRANSFER_RECEIVE, JSON.toJSONString(mqFact));
                }else{
                    rabbitMqProducter.sendToUniversalTransferQueue(mqFact);
                }
            }));
        // 关闭线程池
        pool.shutdown();
