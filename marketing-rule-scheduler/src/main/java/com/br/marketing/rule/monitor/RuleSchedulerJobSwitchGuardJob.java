package com.br.marketing.rule.monitor;
...
/**
 * rule-scheduler服务Job开关守护任务
 * 扫描ruleSchedulerJobOnlineSwitch开关，如果开启超过30分钟则发送告警
 * 注意：此Job在monitor包下，不受JobSwitchRuleAspect切面影响
 * 
 * @author system
 */
@Component
@Slf4j
public class RuleSchedulerJobSwitchGuardJob extends AbstractSimpleElasticJob {
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        try {
            // 获取开关状态
            Boolean jobOnlineSwitch = marketingCommonConfig.getRuleSchedulerJobOnlineSwitch();
            ...
            if (isSwitchOpen) {
                ...
                if (getSwitchOpenTime() == null) {
                    ...
                    log.info("检测到rule-scheduler服务Job开关已开启，开始计时");
                    return;
                }
                ...
                if (openDuration >= ALARM_THRESHOLD_MS) {
                    ...
                    String alarmMessage = String.format(
                        "【重要告警】rule-scheduler服务Job上线开关(ruleSchedulerJobOnlineSwitch)已开启超过%d分钟！" +
                    ...
                    log.warn(AlertLog.buildWarnMessage(
                        ...
                        alarmMessage,
                        "rule-scheduler服务Job开关超时告警"
                    ));
                } else {
                    long minutes = openDuration / (60 * 1000);
                    log.warn("rule-scheduler服务Job开关已开启{}分钟，未达到告警阈值", minutes);
                }
            } else {
                // 开关已关闭
            ...
            log.error("rule-scheduler服务Job开关守护任务执行异常", e);
        }
    }
}
