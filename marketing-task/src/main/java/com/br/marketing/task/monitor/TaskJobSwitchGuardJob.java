package com.br.marketing.task.monitor;
...
/**
 * task服务Job开关守护任务
 * 扫描scoreTaskJobOnlineSwitch开关，如果开启超过30分钟则发送告警
...
@Component
@Slf4j
public class TaskJobSwitchGuardJob extends AbstractSimpleElasticJob {
    ...
    public void process(JobExecutionMultipleShardingContext context) {
        try {
            // 获取开关状态
            Boolean jobOnlineSwitch = marketingCommonConfig.getScoreTaskJobOnlineSwitch();
            ...
            if (isSwitchOpen) {
                ...
                if (getSwitchOpenTime() == null) {
                    ...
                    log.info("检测到task服务Job开关已开启，开始计时");
                    return;
                }
                ...
                if (openDuration >= ALARM_THRESHOLD_MS) {
                    ...
                    String alarmMessage = String.format(
                        "【重要告警】task服务Job上线开关(scoreTaskJobOnlineSwitch)已开启超过%d分钟！" +
                    ...
                    long minutes = openDuration / (60 * 1000);
                    log.warn("task服务Job开关已开启{}分钟，未达到告警阈值", minutes);
                }
            } else {
                // 开关已关闭
            ...
            log.error("task服务Job开关守护任务执行异常", e);
        }
    }
}
